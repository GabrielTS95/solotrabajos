...
for key, _ in FUNCIONALIDADES_JUEZ:
    rows[-1][f"{key}_score"] = eval_juez.get(f"{key}_score", 0)
    rows[-1][f"{key}_justification"] = eval_juez.get(f"{key}_justification", "")
...
<div class="premium-header-sub">
               Generado: {datetime.now().strftime('%d-%m-%Y %H:%M:%S')} &nbsp;|&nbsp; Umbral Global: 80% &nbsp;|&nbsp; Casos De Prueba: {total_cases} &nbsp;|&nbsp; Tiempo Ejecucion: {total_exec_time_formatted}
</div>
</div>
</div>
</div>
